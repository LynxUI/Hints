...
local FRIENDS_BNET_STATUS_TABLE = {L["|cffff0000<AFK>|r"], L["|cffff0000<DND>|r"], ""}
...
ScriptLoader:AddHookScript(QuickJoinToastButton, "OnEnter", function()
    local maxOnlineFriends = FriendsDB:GetMaxOnlineFriends()
    ...
    if (numBNetOnline + numWoWOnline) > 0 then
        ...
        if numWoWOnline > 0 then
            ...
            Tooltip:AddHighlightLine(L["World of Warcraft (%d):"]:format(numWoWOnline))
        ...
        if numBNetOnline > 0 then
            Tooltip:AddEmptyLine()
            Tooltip:AddHighlightLine(L["Battle.net (%d):"]:format(numBNetOnline))
            local status = 0
